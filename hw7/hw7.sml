...
fun preprocess_prog (e : geom_exp) : geom_exp =
    case e of
         LineSegment (aXstart, aYstart, bXend, bYend) =>
           if aXstart > bXend orelse (real_close(aXstart, bXend) andalso aYstart > bYend)
           then LineSegment (bXend, bYend, aXstart, aYstart)
           else if real_close_point (aXstart, aYstart) (bXend, bYend)
    ...
    case e of
       ...
       | Shift (r1, r2, e) =>
           case eval_prog (e, env) of
             ...
             | VerticalLine r => VerticalLine (r + r1)
             | LineSegment (x1, y1, x2, y2) =>
                 LineSegment (x1 + r1, y1 + r2, x2 + r1, y2 + r2)
             | _ => raise BadProgram("shift wrong value")
