...
j = Intersect.new(LineSegment.new(ZERO, ZERO, ZERO, ONE),
				  LineSegment.new(ZERO, ONE, ZERO, TWO))
j1 = j.preprocess_prog.eval_prog([])
if not ((j1.is_a? Point) and j1.x == ZERO and j1.y == ONE)
	puts "LineSegment intersect only touching not working properly"
end

k = Intersect.new(LineSegment.new(ZERO, ZERO, THREE, THREE),
				  LineSegment.new(ONE, ONE, TWO, TWO))
k1 = k.preprocess_prog.eval_prog([])
if not ((k1.is_a? LineSegment) and
		(k1.x1 == ONE and k1.y1 == ONE and k1.x2 == TWO and k1.y2 == TWO))
	puts "LineSegment intersect including not working properly"
end

l = Intersect.new(LineSegment.new(TWO, TWO, ZERO, ZERO),
				  LineSegment.new(ONE, ONE, THREE, THREE))
l1 = l.preprocess_prog.eval_prog([])
if not ((l1.is_a? LineSegment) and
		(l1.x1 == ONE and l1.y1 == ONE and l1.x2 == TWO and l1.y2 == TWO))
	puts "LineSegment intersect overlapping not working properly"
end

m = Intersect.new(LineSegment.new(ZERO, ZERO, ONE, ONE),
				  LineSegment.new(TWO, TWO, THREE, THREE))
m1 = m.preprocess_prog.eval_prog([])
if not (m1.is_a? NoPoints)
	puts "LineSegment intersect disjointing not working properly"
end
